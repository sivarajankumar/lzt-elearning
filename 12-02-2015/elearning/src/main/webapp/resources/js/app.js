...
$( document ).ready(function() {
	...
	$(".request-friend").webuiPopover({title:'Yêu cầu kết bạn', type:'async',cache:false,url:"/elearning/friendNotify", content:function(data){
		alert("duan oi ngon roi");
		return "111111111";
	}});
	...
	$(document).on('click', '.addFriend' ,function() {
		...
		$.ajax({
			...
			contentType : 'application/json',
			dataType : 'json',
			url: "/elearning/addFriend",
			processData: false,
			data: JSON.stringify({toUser:toUser}),
			// data: JSON.stringify({toUser:"1",message:"chanvai"}), // Note it is important
